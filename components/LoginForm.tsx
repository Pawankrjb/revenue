...
export const LoginForm: React.FC<LoginFormProps> = ({ onLogin, onSwitchToSignup, initialError }) => {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Sync initial error from props (e.g. from URL redirect) to local state
  useEffect(() => {
    if (initialError) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Login success is handled by the onAuthStateChange listener in App.tsx
